...
{
	public final ResourceLocation texture = new ResourceLocation(InfoBHS.MOD_ID, "textures/gui/atomic_transformer.png");
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
	{
		...
		if(tile.recipe == null)
			...
			try
			{
				...
				GL11.glScaled(.5, .5, 1);
				fontRenderer.drawString(I18n.translateToLocal("gui." + InfoBHS.MOD_ID + ":rf.required") + ": " + String.format("%,d", r.getEnergyUsed(tile.inventory.getStackInSlot(0))) + " RF", 0, 0, 0, false);
				fontRenderer.drawString(I18n.translateToLocal("gui." + InfoBHS.MOD_ID + ":rf.stored") + ": " + String.format("%,d", tile.stored) + " RF", 0, 18, 0, false);
				fontRenderer.drawString(I18n.translateToLocal("gui." + InfoBHS.MOD_ID + ":input") + ": " + r.getInputItemCount() + "x " + tile.getStackInSlot(0).getDisplayName(), 0, 36, 0, false);
				fontRenderer.drawString(I18n.translateToLocal("gui." + InfoBHS.MOD_ID + ":output") + ": " + InterItemStack.getStackSize(tile.recipe.getOutputItem()) + "x " + tile.recipe.getOutputItem().getDisplayName(), 0, 54, 0, false);
				fontRenderer.drawString(I18n.translateToLocal("gui." + InfoBHS.MOD_ID + ":progress") + ": " + format.format(progress * 100D * 100D) + "%", 0, 72, 0, false);
				GL11.glPopMatrix();
