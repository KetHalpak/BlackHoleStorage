...
public class TileFluidEjector extends TileSyncableTickable implements IFluidHandler, IFluidTankProperties
{
	...
	private FluidStack lastParticleSize = null;
		...
		if(!isValidState())
			...
			b:
			{
				...
				if(lastParticleSize != null && !world.isRemote && world.rand.nextInt(16) < 1)
				{
					...
					BlackHoleStorage.proxy.spawnLiquidTexturedFX(world, tpos.getX() + .5 + fx, tpos.getY() + .5 + fy, tpos.getZ() + .5 + fz, pos.getX() + .5, pos.getY() + (upsideDown ? .75 : .55), pos.getZ() + .5, 1, lastParticleSize, .1F, (int) MathHelper.clip(Math.sqrt(Math.sqrt(lastParticleSize.amount)) * 2.4, 1, 40));
	...
	{
		return world.getBlockState(pos).getBlock() instanceof BlockFluidEjector;
	}
	...
	protected FluidStack drainFluid(int maxExtract, boolean simulate)
	{
		if(scan == null)
			return null;
		...
		if(tile instanceof TileBlackHole)
		{
			...
			if((pkt == EnumBlackHolePacketType.FLUID && hole.getStorage() instanceof BlackHoleStorageFluid) || pkt == null)
			{
				...
				{
					FluidStack fs = (FluidStack) ret;
					if(!simulate)
					{
						if(lastParticleSize == null)
							lastParticleSize = fs.copy();
						else
							lastParticleSize.amount += fs.amount;
					}
					return fs;
				}
			}
	...
	{
		return drainFluid(resource != null ? resource.amount : 0, !doDrain);
	}
	
	@Override
	public FluidStack drain(int maxDrain, boolean doDrain)
	{
		return drainFluid(maxDrain, !doDrain);
	}
	...
	public boolean canDrainFluidType(FluidStack fluidStack)
	{
		...
		if(tile instanceof TileBlackHole)
		{
			...
			{
				BlackHoleStorageFluid fs = (BlackHoleStorageFluid) hole.getStorage();
				return fs.isFluidSupported(fluidStack);
			}
		}
		return false;
	}
